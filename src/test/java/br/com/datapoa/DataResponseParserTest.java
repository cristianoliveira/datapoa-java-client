...
public class DataResponseParserTest {
    ...
    public void testGivenJsonStringWhenParseItShouldReturnInstanceOfClass() {
        ...
        String jsonStringStubClient =
                "{ \"name\" : \"" + expectedName + "\",  \"age\" : \"" + expectedAge
                        + "\", \"isCrazy\" : true }";
        DataResponse mockedResponse = mock(DataResponse.class);
        when(mockedResponse.getJsonString()).thenReturn(jsonStringStubClient);

        // when
        StubResultRequest result =
                new DataResponseParser(mockedResponse).parseTo(StubResultRequest.class);
    ...
    @Test
    public void testGivenRealJsonStringWhenParseItShouldReturnInstanceDataPoaEntity()
            throws IOException, URISyntaxException {
        // given
        ClassLoader classLoader = getClass().getClassLoader();
        String resourcePath =
                classLoader.getResource("json/stub_result_from_datapoa.json").getPath();
